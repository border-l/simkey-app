...
// The mapping field encodes whether SHIFT is required.
// If mapping >= 10000 then SHIFT is needed, and the actual VK code is (mapping % 10000).
// The isMouse field is 1 for mouse buttons.
typedef struct {
    const char* key;
    int mapping;
    int isMouse;
...
KeyMap keyMap[] = {
    // Letters: Uppercase letters require SHIFT (mapping = 10000 + VK code)
    {"A", 10000 + 'A', 0},
    {"B", 10000 + 'B', 0},
    {"C", 10000 + 'C', 0},
    {"D", 10000 + 'D', 0},
    ...
    {"Y", 10000 + 'Y', 0},
    {"Z", 10000 + 'Z', 0},
    // Lowercase letters: no SHIFT required (mapping = VK code)
    {"a", 'A', 0},
    {"b", 'B', 0},
    {"c", 'C', 0},
    {"d", 'D', 0},
    ...
    {"UP", VK_UP, 0},
    {"DOWN", VK_DOWN, 0},
    // Mouse buttons (isMouse = 1)
    {"MB_LEFT", MOUSEEVENTF_LEFTDOWN, 1},
    {"MB_RIGHT", MOUSEEVENTF_RIGHTDOWN, 1},
    {"MB_MIDDLE", MOUSEEVENTF_MIDDLEDOWN, 1},
    ...
    {"(", 10000 + '9', 0},
    {")", 10000 + '0', 0},
    {"_", 10000 + VK_OEM_PLUS, 0},    // SHIFT + "=" yields "_"
    ...
    {"<", 10000 + VK_OEM_COMMA, 0},     // SHIFT + "," yields "<"
    {">", 10000 + VK_OEM_PERIOD, 0},    // SHIFT + "." yields ">"
    {"?", 10000 + VK_OEM_2, 0},         // SHIFT + "/" yields "?"
    // Non-shifted versions of some keys:
    {"=", VK_OEM_PLUS, 0},
    {"-", VK_OEM_MINUS, 0},
    {"`", VK_OEM_3, 0},
    {"[", VK_OEM_4, 0},
    ...
    {".", VK_OEM_PERIOD, 0},
    {"/", VK_OEM_2, 0},
    // Additional mappings (duplicates for clarity)
    {"CMD", VK_LWIN, 0},
    {"WIN", VK_LWIN, 0},
    {"CAPS", VK_CAPITAL, 0},
    {"TAB", VK_TAB, 0},
    {"LIST", VK_APPS, 0},
    {"END", VK_END, 0},
    {"PRNTSCR", VK_SNAPSHOT, 0},
    {"SCRL_LOCK", VK_SCROLL, 0},
    {"PAUSE", VK_PAUSE, 0},
    {"PG_UP", VK_PRIOR, 0},
    {"PG_DOWN", VK_NEXT, 0},
    {"HOME", VK_HOME, 0},
...
void releaseKey(const char* key) {
    ...
    if (km->isMouse) {
        ...
        if (km->mapping == MOUSEEVENTF_LEFTDOWN)
            ...
            releaseEvent = MOUSEEVENTF_MIDDLEDOWN; // Not normally reached
...
void scroll(int amount) {
    // Ensure no modifiers (like SHIFT) are held during scrolling.
    keybd_event(VK_SHIFT, 0, KEYEVENTF_KEYUP, 0);
